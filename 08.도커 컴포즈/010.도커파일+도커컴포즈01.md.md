...
def hello():
    ...
    return f(아 파이썬에선 f가 포맷을 애기한다)'어서오세요! 현재까지 접속 횟수는 {count}번입니다.\n'

if __name__ == "__main__":
    app.run(host="0.0.0.0"(모든서비스를 허용하겠다), port=5000(모든서비스를 5000로 서비스하겠다))
...
WORKDIR /app
COPY . .
RUN pip(apt 같은건데 파이썬에서만 쓰는 명령어) install --no-cache-dir -r requirements.txt --
EXPOSE 5000
CMD ["python", "app.py"]
